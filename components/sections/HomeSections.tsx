    ...
    <Section id="tentang" className="about-section">
      <div className="container about-grid">
        <div className="about-image-wrap">
          <Image src={about.image} alt="Pembelajaran bersama di Ibadurrahman" fill sizes="(max-width: 760px) 100vw, 48vw" />
          <div className="about-stat"><strong>Utuh</strong><span>Ilmu, iman, dan karakter</span></div>
        </div>
...
function CampusesSection() {
  return (
    <Section className="campuses-section">
      <div className="container">
        <SectionHeader eyebrow="Lingkungan Santri" title="Ruang yang aman untuk belajar dan menjadi diri terbaik." align="center" />
        <div className="campuses-grid">{campuses.filter((item) => item.active).map((campus) => <CampusCard key={campus.id} campus={campus} />)}</div>
      </div>
    </Section>
  );
}
  ...
  return (
    <Section className="video-section">
      <div className="container video-grid">
        <div className="video-copy"><SectionHeader eyebrow={video.eyebrow} title={video.title} description={video.description} /><button className="play-link" type="button" onClick={() => setOpen(true)}><span>▶</span> Putar Cerita Pesantren</button></div>
        <button className="video-poster" type="button" aria-label="Putar video kehidupan pesantren" onClick={() => setOpen(true)}><Image src={video.thumbnail} alt="" fill sizes="(max-width: 760px) 100vw, 48vw" /><span>▶</span></button>
      </div>
      {open ? <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Video kehidupan pesantren" onClick={() => setOpen(false)}><div className="video-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" type="button" aria-label="Tutup video" onClick={() => setOpen(false)}>×</button>{video.videoUrl ? <iframe src={video.videoUrl} title="Kehidupan Pesantren" allow="autoplay; fullscreen" /> : <div className="video-empty"><strong>Video siap ditambahkan</strong><p>Isi <code>videoUrl</code> di <code>content/home.ts</code> dengan YouTube, Vimeo, atau MP4.</p></div>}</div></div> : null}
    </Section>
  );
}
...
function GallerySection() {
  ...
  return (
    <Section className="gallery-section">
      <div className="container">
        <div className="section-heading-row"><SectionHeader eyebrow="Cerita Visual" title="Momen-momen yang membentuk perjalanan santri." /><Link className="text-link" href="/gallery">Buka Galeri <span aria-hidden="true">↗</span></Link></div>
...
function ContactSection() {
  return (
    <Section className="contact-strip"><div className="container contact-strip-inner"><div><p className="section-eyebrow">Mari Berkenalan</p><h2>Punya pertanyaan tentang Ibadurrahman?</h2></div><div><p>Tim kami siap membantu keluarga memahami program dan proses pendaftaran.</p><Link className="button" href="/contact">Hubungi Kami <span aria-hidden="true">↗</span></Link></div></div></Section>
  );
}

const sectionMap = { about: AboutSection, values: ValuesSection, cta: CtaSection, programs: ProgramsSection, campuses: CampusesSection, testimonials: TestimonialsSection, video: VideoSection, gallery: GallerySection, contact: ContactSection };
